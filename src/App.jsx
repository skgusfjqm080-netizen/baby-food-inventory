...
const P = {
  ...
  header:   "#FAF6F2",
  rose:     "#E8A598",   // 포인트
  roseDark: "#C08070",   // 서브 포인트
  rosePale: "#F0E6DC",   // 연한 배경
  text:     "#3A2520",
  textSub:  "#C4A090",
  tabOn:    "#E8A598",
  tabOff:   "#F0E6DC",
...
const isExpiringSoon = (m) => {
  const e = addDays(m, VALIDITY_DAYS);
  return e >= todayStr() && e <= addDays(todayStr(), 3);
};
...
export default function App() {
  ...
  // 첫 방문 시 프로필 설정 유도
  useEffect(() => {
    if (!babyName) setShowProfileModal(true);
  }, []);

  const openProfile = () => {
    setProfileDraft({ name: babyName, photo: babyPhoto });
    setShowProfileModal(true);
  };
  const saveProfile = () => {
    setBabyName(profileDraft.name);
    setBabyPhoto(profileDraft.photo);
  ...
  const handlePhotoChange = (e) => {
    ...
    reader.onload = (ev) => setProfileDraft(d => ({ ...d, photo: ev.target.result }));
  ...
  const allWeekStarts  = useCallback(() =>
    Array.from({ length:13 }, (_, i) => addDays(todayWS, (i-8)*7)), [todayWS]);

  useEffect(() => {
    if (mainTab !== "diet" || dietView !== "week") return;
    setTimeout(() => {
      const el = weekRefs.current[todayWS];
      if (el && scrollRef.current)
        scrollRef.current.scrollTo({ top: el.offsetTop - 8, behavior:"instant" });
    }, 60);
  ...
  const openAddMeal  = (d) => { setMealDate(d); setEditMealIdx(null); setMealForm({items:[{name:"",gram:""}]}); setSugg([]); setShowMealModal(true); };
  const openEditMeal = (d,i) => { setMealDate(d); setEditMealIdx(i); setMealForm({items:[...dayMeals(d)[i].items.map(x=>({...x}))]}); setSugg([]); setShowMealModal(true); };
  const saveMeal = () => {
  ...
  const openCD  = (d) => { setCdDate(d); setCdInput(""); setCdSugg([]); setShowCDModal(true); };
  ...
  const updateCDInput = (v) => { setCdInput(v); const q=v.toLowerCase(); setCdSugg(q?cubes.map(c=>c.name).filter(n=>n.toLowerCase().includes(q)):cubes.map(c=>c.name)); };
  ...
  const openAddCube  = () => { setEditCube(null); setCubeForm({name:"",category:"veggie",count:0,made:today,note:""}); setShowCubeForm(true); };
  ...
  const renderDayCard = (dateStr, compact=false) => {
    ...
    if (compact) {
      ...
      return (
        <div key={dateStr} onClick={()=>setSelDate(dateStr)} style={{borderRadius:10,padding:"5px 3px",textAlign:"center",background:isSel?P.rose:isToday?"#FFF0EB":"#fff",border:isToday&&!isSel?`1.5px solid ${P.rose}`:"1.5px solid transparent",cursor:"pointer",minHeight:54}}>
      ...
      <div key={dateStr} style={{margin:"0 12px 8px",background:P.surface,borderRadius:16,border:isToday?`2px solid ${P.rose}`:"2px solid transparent",boxShadow:"0 1px 6px rgba(196,112,80,0.07)"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 14px 6px"}}>
          ...
          <div style={{display:"flex",gap:6}}>
            <button onClick={()=>openCD(dateStr)} style={{padding:"4px 10px",borderRadius:10,border:`1.5px solid #8B5CF6`,background:"#F5F3FF",color:"#8B5CF6",fontSize:11,fontWeight:700,cursor:"pointer"}}>🧊 큐브</button>
  ...
  return (
    <div style={{fontFamily:"'Noto Sans KR',sans-serif",background:P.bg,minHeight:"100vh",maxWidth:480,margin:"0 auto",paddingBottom:80}}>
      ...
      {/* ── 헤더 ── */}
      <div style={{background:P.header,borderBottom:`1px solid ${P.border}`,padding:"16px 16px 0"}}>
        {/* 프로필 행 */}
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14}}>
          ...
          <div onClick={openProfile} style={{width:54,height:54,borderRadius:"50%",overflow:"hidden",border:`2.5px solid ${P.rose}`,flexShrink:0,cursor:"pointer",background:P.rosePale,display:"flex",alignItems:"center",justifyContent:"center"}}>
            {babyPhoto
              ? <img src={babyPhoto} alt="아기" style={{width:"100%",height:"100%",objectFit:"cover"}}/>
              : <BabyIllust size={54}/>
            }
          </div>
          <div style={{flex:1}}>
            <div style={{color:P.rose,fontSize:10,fontWeight:700,letterSpacing:".06em"}}>BABY DIARY</div>
            <div style={{color:P.text,fontSize:17,fontWeight:900,marginTop:1,lineHeight:1.2}}>
              {babyName ? `${babyName} 이유식 다이어리` : "이유식 다이어리"}
            </div>
            <div style={{color:P.textSub,fontSize:10,marginTop:2}}>{today.replace(/-/g,".")} · 우리 아기 식단 기록</div>
          </div>
          {/* 설정 버튼 */}
          <button onClick={openProfile} style={{width:32,height:32,borderRadius:"50%",border:`1px solid ${P.border}`,background:"#fff",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
        ...
        <div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 155px)"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"flex-end",padding:"10px 16px 4px",gap:6}}>
            ...
            {[{id:"week",label:"주간"},{id:"month",label:"월간"}].map(v=>(
              <button key={v.id} onClick={()=>setDietView(v.id)} style={{padding:"5px 14px",borderRadius:20,border:"none",background:dietView===v.id?P.rose:P.rosePale,color:dietView===v.id?"#fff":P.roseDark,fontSize:12,fontWeight:700,cursor:"pointer"}}>
                {v.label}
              </button>
            ))}
          </div>
              ...
              {allWeekStarts().map(ws=>{
                ...
                return (
                  <div key={ws} ref={el=>{if(el)weekRefs.current[ws]=el;}}>
                    <div style={{display:"flex",alignItems:"center",padding:"12px 16px 6px",position:"sticky",top:0,background:P.bg,zIndex:10}}>
          ...
          {dietView==="month"&&(
            <div style={{overflowY:"auto",flex:1,padding:"0 12px"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",textAlign:"center",marginBottom:4}}>
                {DAYS_KO.map((d,i)=>(
                  <div key={d} style={{fontSize:11,fontWeight:700,color:i===0?"#E53935":i===6?"#3B82F6":P.textSub,padding:"4px 0"}}>{d}</div>
                ))}
              </div>
              ...
              <div style={{background:P.surface,borderRadius:16,padding:"14px",boxShadow:"0 2px 10px rgba(196,112,80,0.08)",marginBottom:16}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
                  ...
                  <div style={{display:"flex",gap:6}}>
                    <button onClick={()=>openCD(selDate)} style={{padding:"5px 10px",borderRadius:10,border:"1.5px solid #8B5CF6",background:"#F5F3FF",color:"#8B5CF6",fontSize:11,fontWeight:700,cursor:"pointer"}}>🧊 큐브</button>
                ...
                {cdList(selDate).length>0&&(
                  <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:8}}>
                    {cdList(selDate).map(n=>(
                      <span key={n} style={{fontSize:11,background:"#F5F3FF",color:"#7C3AED",borderRadius:8,padding:"3px 10px",fontWeight:700,display:"flex",alignItems:"center",gap:4}}>
                        🧊 {n} 큐브 만드는 날
                        <span onClick={()=>removeCD(selDate,n)} style={{cursor:"pointer",color:"#bbb",fontSize:12}}>×</span>
                      </span>
                    ))}
                  </div>
                )}
                {dayMeals(selDate).length===0&&cdList(selDate).length===0&&(
                  <div style={{textAlign:"center",padding:"20px 0",color:"#DDD0C8",fontSize:13}}>기록이 없어요</div>
                )}
        ...
        <div>
          ...
          {cubeTab==="stock"&&(
            <div style={{padding:"14px 14px 0"}}>
              <div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:8,scrollbarWidth:"none"}}>
                ...
                {CATEGORIES.map(c=>(
                  <button key={c.id} onClick={()=>setFilterCat(c.id)} style={{flexShrink:0,padding:"5px 12px",borderRadius:20,border:"none",background:filterCat===c.id?c.color:P.rosePale,color:filterCat===c.id?"#fff":P.roseDark,fontSize:12,fontWeight:600,cursor:"pointer"}}>{c.label}</button>
                ))}
              </div>
          ...
          {cubeTab==="alert"&&(
            <div style={{padding:14}}>
              {lowCubes.length===0?(
                <div style={{textAlign:"center",padding:"60px 0",color:P.textSub}}>
                  <div style={{fontSize:44}}>✅</div>
                  <div style={{fontSize:14,fontWeight:700,color:P.text,marginTop:10}}>부족한 재료가 없어요!</div>
                  <div style={{fontSize:12,color:P.textSub,marginTop:4}}>모든 큐브가 충분해요 😊</div>
                </div>
              ):(
                <>
        ...
        <div style={overlayStyle}>
          <div style={{...modalStyle, maxHeight:"65vh"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:10,marginBottom:20}}>
              <div onClick={()=>photoInputRef.current?.click()} style={{width:80,height:80,borderRadius:"50%",overflow:"hidden",border:`3px solid ${P.rose}`,cursor:"pointer",background:P.rosePale,display:"flex",alignItems:"center",justifyContent:"center"}}>
                {profileDraft.photo
                  ? <img src={profileDraft.photo} alt="미리보기" style={{width:"100%",height:"100%",objectFit:"cover"}}/>
                  : <BabyIllust size={80}/>
                }
              </div>
              <button onClick={()=>photoInputRef.current?.click()} style={{fontSize:12,padding:"5px 16px",borderRadius:10,border:`1px solid ${P.border}`,background:"#fff",color:P.roseDark,cursor:"pointer"}}>
                📷 사진 선택
              </button>
            ...
            <input
              ...
              placeholder="예: 지우, 민준"
              style={{...inputSt,width:"100%",marginBottom:20}}
            />
            <button onClick={saveProfile} style={{...saveSt,background:P.rose}}>저장하기</button>
            {babyName&&<button onClick={()=>setShowProfileModal(false)} style={{width:"100%",padding:"12px",borderRadius:14,border:`1px solid ${P.border}`,background:"#fff",color:P.textSub,fontSize:14,cursor:"pointer",marginTop:8}}>취소</button>}
          </div>
        </div>
      )}
      ...
      {showMealModal&&(
        <div style={overlayStyle} onClick={()=>setShowMealModal(false)}>
          <div onClick={e=>e.stopPropagation()} style={modalStyle}>
            ...
            <button onClick={addMealItem} style={{width:"100%",padding:"9px",borderRadius:10,border:`1.5px dashed ${P.rose}`,background:"transparent",color:P.rose,fontSize:13,fontWeight:700,cursor:"pointer",marginBottom:12,marginTop:4}}>+ 재료 추가</button>
          ...
          <div onClick={e=>e.stopPropagation()} style={modalStyle}>
            ...
            {cdList(cdDate).length>0&&(
              <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:12}}>
                {cdList(cdDate).map(n=>(
                  <span key={n} style={{fontSize:12,background:"#F5F3FF",color:"#7C3AED",borderRadius:10,padding:"5px 12px",fontWeight:700,display:"flex",alignItems:"center",gap:6}}>
                    🧊 {n}<span onClick={()=>removeCD(cdDate,n)} style={{cursor:"pointer",color:"#bbb",fontSize:14}}>×</span>
                  </span>
                ))}
              </div>
            )}
            <div style={{position:"relative",marginBottom:10}}>
              <div style={{display:"flex",gap:8}}>
                <input value={cdInput} onChange={e=>updateCDInput(e.target.value)} onFocus={()=>setCdSugg(cubes.map(c=>c.name))} onBlur={()=>setTimeout(()=>setCdSugg([]),200)} placeholder="재료명 입력 또는 선택" style={{...inputSt,flex:1}} onKeyDown={e=>e.key==="Enter"&&addCD()}/>
                <button onClick={addCD} style={{padding:"0 16px",borderRadius:12,border:"none",background:"#8B5CF6",color:"#fff",fontSize:14,fontWeight:700,cursor:"pointer"}}>추가</button>
              </div>
              {cdSugg.length>0&&(
                <div style={{position:"absolute",top:"100%",left:0,right:0,background:"#fff",border:"1.5px solid #8B5CF6",borderRadius:12,zIndex:300,boxShadow:"0 4px 16px rgba(0,0,0,0.1)",maxHeight:160,overflowY:"auto"}}>
                  {cdSugg.map(n=>(
                    <div key={n} onMouseDown={()=>{setCdInput(n);setCdSugg([]);}} onTouchStart={()=>{setCdInput(n);setCdSugg([]);}} style={{padding:"10px 14px",fontSize:14,color:P.text,cursor:"pointer",borderBottom:`1px solid ${P.bg}`}}>🧊 {n}</div>
                  ))}
                </div>
              )}
            </div>
            <button onClick={()=>setShowCDModal(false)} style={{...saveSt,background:"#8B5CF6"}}>완료</button>
          </div>
        </div>
      )}
      ...
      {showCubeForm&&(
        <div style={overlayStyle} onClick={()=>setShowCubeForm(false)}>
          <div onClick={e=>e.stopPropagation()} style={{...modalStyle, maxHeight:"55vh"}}>
            ...
            <input value={cubeForm.name} onChange={e=>setCubeForm({...cubeForm,name:e.target.value})} placeholder="예: 당근, 소고기" style={{...inputSt,width:"100%",marginBottom:12}}/>
            <label style={labelSt}>카테고리</label>
            <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:12}}>
              {CATEGORIES.map(c=>(
                <button key={c.id} onClick={()=>setCubeForm({...cubeForm,category:c.id})} style={{padding:"5px 10px",borderRadius:10,border:"none",background:cubeForm.category===c.id?c.color:P.rosePale,color:cubeForm.category===c.id?"#fff":P.roseDark,fontSize:11,fontWeight:600,cursor:"pointer"}}>{c.label}</button>
              ))}
            </div>
            <div style={{display:"flex",gap:10,marginBottom:12}}>
              <div style={{flex:1}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:6}}>
                  <button onClick={()=>setCubeForm({...cubeForm,count:Math.max(0,cubeForm.count-1)})} style={{width:34,height:34,borderRadius:10,border:"none",background:P.rosePale,fontSize:18,fontWeight:700,cursor:"pointer",color:P.roseDark}}>−</button>
                  <input type="number" inputMode="numeric" value={cubeForm.count} onChange={e=>setCubeForm({...cubeForm,count:Number(e.target.value)})} style={{width:50,textAlign:"center",padding:"8px 4px",borderRadius:10,border:`1.5px solid ${P.border}`,fontSize:16,fontWeight:700,outline:"none"}}/>
                  <button onClick={()=>setCubeForm({...cubeForm,count:cubeForm.count+1})} style={{width:34,height:34,borderRadius:10,border:"none",background:P.rose,fontSize:18,fontWeight:700,cursor:"pointer",color:"#fff"}}>+</button>
                </div>
              </div>
              <div style={{flex:1}}>
                ...
                <div style={{fontSize:11,color:P.textSub,marginTop:4}}>유효기간: <span style={{color:P.rose,fontWeight:700}}>{addDays(cubeForm.made,VALIDITY_DAYS)}</span></div>
              </div>
            </div>
            <label style={labelSt}>메모 (선택)</label>
            <input value={cubeForm.note} onChange={e=>setCubeForm({...cubeForm,note:e.target.value})} placeholder="알레르기, 아기 반응 등" style={{...inputSt,width:"100%",marginBottom:16}}/>
...
const saveSt  = {width:"100%",padding:"14px",borderRadius:14,border:"none",color:"#fff",fontSize:16,fontWeight:800,cursor:"pointer"};
